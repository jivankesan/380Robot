...
static constexpr double MOTOR_MAX_RPM = 210.0;   // → ~1.056 m/s top speed
...
// ROI is a lookahead window (y: 0.10–0.60); errors are larger/earlier than
// a centroid-only approach, so KD is higher to damp the lookahead overshoot.
...
// Drop zone approach: pivot right, then stop and drop
...
// Find line after drop: reverse to clear the package, then turn left to reacquire
...
// Approach params (vision-based centering on blue circle – unused, kept for reference)
