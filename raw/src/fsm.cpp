...
static std::string to_str(State s) {
  switch (s) {
    ...
    case State::APPROACH_DROP_ZONE:
      return "APPROACH_DROP_ZONE";
    case State::DROP:
      return "DROP";
    case State::FIND_LINE:
      return "FIND_LINE";
    case State::FINAL_FOLLOW:
      return "FINAL_FOLLOW";
    case State::DONE:
      return "DONE";
...
struct FsmCtx {
  ...
  void transition(State next) {
    ...
    if (
      next == State::FOLLOW_LINE_SEARCH || next == State::RETURN_FOLLOW_LINE ||
      next == State::SPIN_180) {
...
static void handle_follow_line_search(FsmCtx& ctx, SharedState& state) {
  ...
  if (ctx.det.valid) {
    std::cout << "[fsm] blue detected (cx=" << ctx.det.cx << " cy=" << ctx.det.cy
              << " h=" << ctx.det.h << ") – driving forward " << PICKUP_DRIVE_TIME_S
              << "s then closing\n";
...
static void handle_return_follow_line(FsmCtx& ctx, SharedState& state) {
  ...
  if (ctx.green_det.valid) {
    last_ret_log = -1.0;
    std::cout << "[fsm] green drop zone seen (cx=" << ctx.green_det.cx << " cy=" << ctx.green_det.cy
              << ") – approaching\n";
...
static void handle_approach_drop_zone(FsmCtx& ctx, SharedState& state) {
  ...
  if (t < DROP_ZONE_TURN_TIME_S) {
    ...
    set_direct_pwm(state, 0, 90);
    static double last_log = -1.0;
    if (t - last_log >= 0.2) {
      last_log = t;
      std::cout << "[fsm] APPROACH_DROP turning right (t=" << t << "s / " << DROP_ZONE_TURN_TIME_S
                << "s)\n";
    }
  } else {
    stop(state);
...
static void handle_find_line(FsmCtx& ctx, SharedState& state) {
  ...
  if (t < FIND_LINE_REVERSE_TIME_S) {
    ...
    std::cout << "[fsm] FIND_LINE: reversing (t=" << t << "s / " << FIND_LINE_REVERSE_TIME_S
              << "s)\n";
    return;
  }
...
static void handle_final_follow(FsmCtx& ctx, SharedState& state) {
  ...
  double line_loss =
    duration_cast<duration<double>>(Clock::now() - ctx.last_line_seen_final).count();
...
void fsm_thread(SharedState& state) {
  ...
  while (!state.shutdown.load()) {
    ...
    if (ctx.line.valid)
      ctx.last_line_valid_time = Clock::now();
    ...
    if (ctx.line.valid)
      ctx.line_ever_seen = true;

    bool check_line =
      (ctx.current == State::FOLLOW_LINE_SEARCH || ctx.current == State::RETURN_FOLLOW_LINE);
    if (check_line && (ctx.line_ever_seen || ctx.current == State::RETURN_FOLLOW_LINE)) {
      double line_loss =
        duration_cast<duration<double>>(Clock::now() - ctx.last_line_valid_time).count();
      if (line_loss > LINE_LOSS_TIMEOUT_S) {
    ...
    switch (ctx.current) {
      case State::INIT:
        ...
        break;
      case State::FOLLOW_LINE_SEARCH:
        handle_follow_line_search(ctx, state);
        break;
      case State::PICKUP:
        handle_pickup(ctx, state);
        break;
      case State::SPIN_180:
        handle_spin180(ctx, state);
        break;
      case State::RETURN_FOLLOW_LINE:
        handle_return_follow_line(ctx, state);
        break;
      case State::APPROACH_DROP_ZONE:
        handle_approach_drop_zone(ctx, state);
        break;
      case State::DROP:
        handle_drop(ctx, state);
        break;
      case State::FIND_LINE:
        handle_find_line(ctx, state);
        break;
      case State::FINAL_FOLLOW:
        handle_final_follow(ctx, state);
        break;
      case State::DONE:
        handle_done(ctx, state);
        break;
      case State::FAILSAFE_STOP:
        handle_failsafe(ctx, state);
        break;
    }
  }
